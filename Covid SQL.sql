...
SELECT
	...
	CASE
		WHEN SUM (new_cases) = 0 THEN NULL
		ELSE SUM (CAST (new_deaths AS REAL)) / SUM (new_cases) * 100
	END AS uk_death_percentage
FROM CovidDeaths
WHERE location LIKE '%KINGDOM%'
GROUP BY [location] 
ORDER BY uk_death_percentage DESC, new_cases desc
...
SELECT TOP 10
	...
	[population],
	SUM (CAST (new_cases AS REAL)) / [population] * 100 AS infected_population_percentage
FROM CovidDeaths
WHERE continent = 'europe' 
GROUP BY [location], [population] 
ORDER BY infected_population_percentage DESC, new_cases desc


SELECT
	[date],
	SUM (total_cases) AS europe_total_cases
	--MAX (CAST (total_cases AS REAL)) / [population] * 100 AS infected_population_percentage
FROM CovidDeaths
WHERE continent = 'europe'
GROUP BY [date]
ORDER BY date ASC, total_cases desc
